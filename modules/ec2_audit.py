...
def has_unencrypted_volumes(ec2_client, instance):
    volumes = [mapping["Ebs"]["VolumeId"] for mapping in instance.get("BlockDeviceMappings", [])]
    if not volumes:
        return False
    vol_data = ec2_client.describe_volumes(VolumeIds=volumes)["Volumes"]
    return any(not vol["Encrypted"] for vol in vol_data)
